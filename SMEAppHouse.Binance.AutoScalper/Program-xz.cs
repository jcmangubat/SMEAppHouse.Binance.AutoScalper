...
/*
            ...
            var depthResult = publicRestClient.Depth(symbol).Result;

            //Get only the first 10 Orderbook records
            var limitedDepthResult = publicRestClient.Depth(symbol, 10).Result;
            ...
            var aggregateTrades = publicRestClient.AggregateTrades(symbol).Result;
...
*/
            ...
            var kLines = publicRestClient.KLines(symbol, KlineInterval.Minute1).Result;
            var closingStartTime = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
            foreach (var item in kLines)
            {
                var unixDate = item.CloseTime;
                var localTime = closingStartTime.AddMilliseconds(unixDate).ToLocalTime().ToString("MM/dd/yyyy HH:mm");
            ...
            var dailyTicker = publicRestClient.TickerDaily("YFIUSDT").Result;

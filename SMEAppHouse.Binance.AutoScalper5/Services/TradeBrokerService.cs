...
namespace SMEAppHouse.Binance.AutoScalper5.Services
{
    public class TradeBrokerService : TopshelfSocketBase<TradeBrokerService>
    {
        ...
        private readonly List<TradePairSetting> _tradePairSettings;
        ...
        public TradeBrokerService(BinanceApiKeys apiKeys, List<TradePairSetting> tradePairSettings)
            : base(TimeSpan.FromSeconds(1), null, true, true)
        {
            _apiKeys = apiKeys;
            _tradePairSettings = tradePairSettings;
        }
        ...
        {
            if (DateTime.Now.Second == tradePairSettings.TradeTimeFrameInSeconds)
            {
